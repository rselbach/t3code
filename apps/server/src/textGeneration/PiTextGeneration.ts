  ...
  const runPiPrompt = (input: {
    ...
    Effect.gen(function* () {
      ...
      if (!piResponseSucceeded(promptResponse, "prompt")) {
        const processExited = yield* transport.isClosed;
        return yield* new TextGenerationError({
          operation: input.operation,
          detail: processExited
            ? "Pi RPC process exited before completing the request."
            : "Pi rejected the text-generation prompt.",
        });
      }
