...
describe("extractPiToolResultText", () => {
  it("reads bare strings and content text blocks", () => {
    expect(extractPiToolResultText("hello")).toBe("hello");
    expect(
      extractPiToolResultText({
        content: [{ type: "text", text: "partial output so far..." }],
        details: { truncation: null },
      }),
    ).toBe("partial output so far...");
  });

  it("joins multi-block content and falls back to stdout/stderr", () => {
    expect(
      extractPiToolResultText({
        content: [
          { type: "text", text: "a" },
    ...
    expect(extractPiToolResultText({ stdout: "out", stderr: "err" })).toBe("out\nerr");
  });

  it("returns null for empty or unrecognized shapes", () => {
    expect(extractPiToolResultText(undefined)).toBeNull();
    expect(extractPiToolResultText({})).toBeNull();
    expect(extractPiToolResultText({ content: [] })).toBeNull();
  });
});
...
describe("piModelCapabilities", () => {
  it("exposes only the thinking levels supported by a reasoning model", () => {
    ...
    const optionIds = (capabilities.optionDescriptors ?? []).flatMap((descriptor) =>
      descriptor.type === "select" ? descriptor.options.map((option) => option.id) : [],
    );
    expect(optionIds).toEqual(["off", "minimal", "low", "medium", "high", "xhigh"]);
  });

  it("uses Pi's clamped medium level as the default", () => {
    const capabilities = piModelCapabilities({
    ...
    const options = (capabilities.optionDescriptors ?? []).flatMap((descriptor) =>
      descriptor.type === "select" ? descriptor.options : [],
    );
    expect(options.map((option) => option.id)).toEqual(["low", "high", "max"]);
    expect(options.find((option) => option.isDefault)?.id).toBe("high");
  });

  it("exposes no option descriptors for non-reasoning models", () => {
    expect(
      piModelCapabilities({ provider: "openai", id: "gpt-4o", reasoning: false })
        .optionDescriptors ?? [],
    ).toEqual([]);
  });
});
...
describe("extractPiSessionState", () => {
  it("reads the active model, thinking level, and streaming flag", () => {
    expect(
      extractPiSessionState(
        asResponse({
          type: "response",
...
describe("describePiCommandFailure", () => {
  it("prefers Pi's error string and describes timeouts", () => {
    expect(describePiCommandFailure(undefined, "prompt")).toContain("timed out");
    expect(
      describePiCommandFailure(
        asResponse({
          type: "response",
    ...
    ).toBe("Pi rejected 'prompt': Agent is already processing");
  });
});
...
describe("extractSessionFile", () => {
  ...
  it("returns undefined for missing / unsuccessful / empty responses", () => {
    ...
    expect(
      extractSessionFile(
        asResponse({ type: "response", command: "get_state", success: false, data: {} }),
      ),
    ).toBeUndefined();
    expect(
      extractSessionFile(
...
describe("piResponseHasCommand", () => {
  const commandsResponse = (names: ReadonlyArray<string>, sourcePath?: string) =>
    asResponse({
      type: "response",
  ...
  it("requires the command to come from the expected extension path", () => {
    expect(
      piResponseHasCommand(
        commandsResponse(["t3-approval-gate"], "/tmp/t3-approvals.ts"),
        "t3-approval-gate",
        "/tmp/t3-approvals.ts",
      ),
    ).toBe(true);
    expect(
      piResponseHasCommand(
        commandsResponse(["t3-approval-gate"], "/tmp/spoof.ts"),
        "t3-approval-gate",
        "/tmp/t3-approvals.ts",
      ),
    ).toBe(false);
