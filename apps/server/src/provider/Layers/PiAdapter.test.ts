...
describe("summarizePiToolOutput / buildPiToolActivityData", () => {
  it("collapses whitespace and truncates long tool output", () => {
    expect(summarizePiToolOutput("  hello\nworld  ")).toBe("hello world");
    expect(summarizePiToolOutput("x".repeat(500), 20)).toBe(`${"x".repeat(19)}…`);
  });

  it("builds work-log friendly tool data with command + raw output", () => {
    expect(
      buildPiToolActivityData({
        toolCallId: "call-1",
        toolName: "bash",
      ...
      toolName: "bash",
      input: { command: "ls -la" },
      item: { command: "ls -la", input: { command: "ls -la" } },
      rawOutput: { content: "README.md\npackage.json" },
    });
  });
});
