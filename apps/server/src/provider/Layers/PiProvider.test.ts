...
const EMPTY_PI_SCRIPT = [
  "#!/bin/sh",
  'case "$1" in',
  '  --version) printf "pi 0.83.0\\n"; exit 0 ;;',
...
const HEALTHY_PI_SCRIPT = [
  ...
  'case "$1" in',
  '  --version) printf "pi 0.83.0\\n"; exit 0 ;;',
  '  *) printf \'{"type":"response","command":"get_available_models","id":"pi-model-discovery","success":true,"data":{"models":[{"provider":"anthropic","id":"claude-sonnet-4-6","name":"Claude Sonnet 4.6","reasoning":true}]}}\\n\'; exit 0 ;;',
  "esac",
  "",
].join("\n");
...
it.layer(NodeServices.layer)("checkPiProviderStatus", (it) => {
  ...
  it.effect("requires Pi 0.83.0 or newer", () =>
    Effect.gen(function* () {
      const snapshot = yield* Effect.scoped(
        Effect.gen(function* () {
          const fs = yield* FileSystem.FileSystem;
      ...
      expect(snapshot.status).toBe("error");
      expect(snapshot.message).toMatch(/0\.83\.0 or newer/i);
    }),
  );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
          ...
          Effect.gen(function* () {
            ...
            yield* fs.writeFileString(
              ...
              [
                "#!/bin/sh",
                'case "$1" in',
                '  --version) sleep 5; printf "pi 0.83.1\\n"; exit 0 ;;',
                '  *) printf \'{"type":"response","command":"get_available_models","id":"pi-model-discovery","success":true,"data":{"models":[]}}\\n\'; exit 0 ;;',
                "esac",
                "",
              ].join("\n"),
        ...
        expect(snapshot.version).toBe("0.83.1");
        expect(snapshot.status).toBe("warning");
      }),
    10_000,
  );
    ...
    Effect.gen(function* () {
      const snapshot = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          const piPath = path.join(dir, "pi");
          yield* fs.writeFileString(piPath, EMPTY_PI_SCRIPT);
          yield* fs.chmod(piPath, 0o755);
          return yield* checkPiProviderStatus(
            decodePiSettings({ enabled: true, binaryPath: piPath }),
            dir,
