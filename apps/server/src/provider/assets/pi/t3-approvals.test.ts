...
describe("t3-approvals: autoApprovedTools (default-deny allowlist)", () => {
  ...
  it("adds edit tools only in auto-accept-edits mode; bash still gated", () => {
    ...
    expect(allowed.has("multi_edit")).toBe(false);
    expect(allowed.has("apply_patch")).toBe(false);
  ...
  it("treats unknown / custom / MCP tools as NOT auto-approved (default-deny)", () => {
    const allowed = autoApprovedTools("auto-accept-edits");
    for (const tool of ["glob", "foobar", "mcp__server__write", "rm", "move", ""]) {
      expect(allowed.has(tool)).toBe(false);
    }
  });
});

describe("t3-approvals: isTrustedBuiltinTool", () => {
  const makeApi = (sourceInfo: {
    readonly source: string;
    readonly path: string;
  }): PiExtensionAPI =>
    ({
      getAllTools: () => [{ name: "read", sourceInfo }],
    }) as unknown as PiExtensionAPI;

  it("trusts only Pi's built-in tool definition", () => {
    expect(
      isTrustedBuiltinTool(makeApi({ source: "builtin", path: "<builtin:read>" }), "read"),
    ).toBe(true);
    expect(isTrustedBuiltinTool(makeApi({ source: "project", path: "/tmp/read.ts" }), "read")).toBe(
      false,
    );
    expect(
      isTrustedBuiltinTool(makeApi({ source: "builtin", path: "<builtin:bash>" }), "read"),
    ).toBe(false);
  });
});
