...
Install Pi 0.83.0 or newer and make sure it is on your `PATH`:
...
~/.pi/agent/settings.json   defaults, enabled-model patterns, packages, theme
```
...
extension. Tool approval is selected through T3 Code's session/runtime approval mode, not a
Pi provider setting — pick it from the runtime-mode control in the composer (for example
**Supervised** or **Auto-accept edits**) or in the project thread options. Pi does not have
T3 Code's automatic reviewer, so **Auto** safely falls back to explicit approvals. **Full
access** turns approval off.

When the active session/runtime mode requires approval, T3 Code gates every tool call that
is not read-only.
...
T3 Code will not run an ungated Pi session when the active mode requires approval. Before
allowing a tool-capable turn, it verifies that the approval extension actually loaded. If
the gate cannot be guaranteed active, T3 Code refuses to start the session rather than run
Pi with unguarded tools — there is no silent fallback to read-only or any other mode.

If you select **Full access** for the session, Pi runs tools without asking. Only do this
in environments where that is acceptable.

## T3 Code MCP Bridge

T3 Code can inject its authenticated, thread-scoped MCP bridge when Pi loads a compatible
`pi-mcp-adapter` extension. T3 Code probes once per working directory for
transient-configuration support. When available, it passes the bridge endpoint and credential to Pi for that process only and
verifies that the extension consumed them before sending the first prompt. The credential
is not added to Pi's persistent MCP configuration.

Without a compatible extension, Pi continues normally without the T3 Code bridge. Other
MCP servers configured in Pi are unaffected. In supervised modes, calls through the
adapter's custom `mcp` tool require approval like other custom tools.
...
- **Approval grants are per call.** Pi's confirmation protocol does not have a persistent
  approve-for-session response, so future calls can ask again.
